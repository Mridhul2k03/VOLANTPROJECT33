...
from .forms import AdminOrderForm, CartForm, ContactForm, FootwearimagesForm, FootwearsForm, OrderForm, UserContactForm, UserRegisterForm,UserLoginForm,FootwearSizeForm
...
from .models import ContactModel, Footwears,Footwearimages,CartModel,Footwearsize, OrderModel, UserContactModel
...
class UserSearchResults(ListView):
    template_name = 'usersearchresults.html'
    model = Footwears
    context_object_name = 'all_search_results'

    def get_queryset(self):
       result = super(UserSearchResults, self).get_queryset()
       query = self.request.GET.get('search')
       if query:
          postresult = Footwears.objects.filter(Q(product_name__contains=query) | Q(product_types__contains=query) | Q(product_category__contains=query) | Q(product_types__contains=query) )
          result = postresult
       else:
           result = None
       return result

class NonUserSearchResults(ListView):
    template_name = 'nonusersearch.html'
    model = Footwears
    context_object_name = 'all_search_results'

    def get_queryset(self):
       result = super(NonUserSearchResults, self).get_queryset()
       query = self.request.GET.get('search')
       if query:
          postresult = Footwears.objects.filter(Q(product_name__contains=query) | Q(product_types__contains=query) | Q(product_category__contains=query) | Q(product_types__contains=query) )
          result = postresult
...
class UserMessagesforAdmin(ListView):
    template_name = 'adminmessages.html'
    model = UserContactModel
    context_object_name = 'messages'
    

class MessagesforAdmin(ListView):
    template_name = 'adminmessages1.html'
    model = ContactModel
    context_object_name = 'messages'
    

# class MessageReplayforUser(View):
#     def get(self, request, *args, **kwargs):
#         id = kwargs.get('id')
#         message = UserContactModel.objects.get(id = id)
#         return render(request, 'adminmessagereplay.html', {'message': message})
    
#     def post(self, request, *args, **kwargs):
#         id = kwargs.get('id')
#         message = UserContactModel.objects.get(id = id)
#         message.message = f'Thank you for your message. We will contact you later.'
#         message.save()
        
#         subject = f'Replay to your message on Volant'
#         message = f'Dear {message.name},\n\nThank you for reaching out to us. We have received your message and we will contact you shortly.'
#         from_email = settings.EMAIL_HOST_USER
